...
read_data = function(i){
  X_initial = read.csv(file = paste0("t60/X_data_n100_t60_rep",i,".csv"), header = T)
  Y = as.matrix(read.csv(file = paste0("t60/Y_data_n100_t60_rep",i,".csv"), header = T))
  T_val = 60
  X = list()
  range = nrow(X_initial)/60
  k = 1
  for(j in 1:range){
...
filename = paste("RESULTS/results_n100_t60_rep", r, ".rda", sep = "")
