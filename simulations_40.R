...
read_data = function(i){
  X_initial = read.csv(file = paste0("t40/X_data_n100_t40_rep",i,".csv"), header = T)
  Y = as.matrix(read.csv(file = paste0("t40/Y_data_n100_t40_rep",i,".csv"), header = T))
  T_val = 40
  X = list()
  range = nrow(X_initial)/40
  k = 1
  for(j in 1:range){
...
filename = paste("RESULTS/results_n100_t40_rep", r, ".rda", sep = "")
