# Rscript R/DATA_clean_real_data.R
rm(list = ls())
library(dplyr)
library(stats)
library(recipes)
...
# update some stuff as factors
data$Marketing_Airline_Network = as.factor(data$Marketing_Airline_Network)
...
# ## try to get rid of some of this
...
save(X_list, file = "2018dataX.Rda")
save(Z, file = "2018Z.Rda")
...
# results = gibbs_sampler_r(data_gibbs = data_actual, B = 1000, n_to_store = 500)
...
# betas
# 
# # look at prediction values of y
# y_pred = colMeans(results$mu) + colMeans(results$g)
# 
# idx = sample(1:end, 1)
# Z2 = Z[idx,]
...
# plot(pred$actual, pred$predicted)
# 
# 
# 
