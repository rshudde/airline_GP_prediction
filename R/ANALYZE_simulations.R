...
#   n_datasets_rep = seq(100, 400, by = 100)
#   
# } else {
...
for (idx in 1:length(n_datasets_rep))
{
  for (i in 1:50)
  {
    ...
    temp_filename = paste("~/Desktop/Airplane_Paper/RESULTS_VARIED/results", n_datasets_rep[idx], "_", i, ".rda", sep = "")
    
    data = generate_simulation_data(n_datasets = n_datasets_rep[idx], n_time = time_reps[idx], n_covariates = 15, seed = 1, seed2 = i)
...
for (i in 1:length(n_datasets_rep))
{
  ...
  write.csv(beta_truth[[i]], row.names = FALSE, file = paste(write_filename, "beta_truth_", n_datasets_rep[i], ".csv", sep = ""))
  ...
  write.csv(g_mu[[i]], row.names = FALSE, file = paste(write_filename, "gpmu_", n_datasets_rep[i], ".csv", sep = ""))
